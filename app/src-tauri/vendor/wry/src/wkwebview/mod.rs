...
pub fn platform_webview_version() -> Result<String> {
  // iOS does not expose the macOS WebKit framework bundle reliably. On recent
  // simulators `NSBundle::bundleWithIdentifier("com.apple.WebKit")` can trap
  // inside CoreFoundation before returning. Tauri only uses this value for
  // diagnostics, so report the platform WebView name without touching NSBundle.
  #[cfg(target_os = "ios")]
  return Ok("WKWebView".to_string());
